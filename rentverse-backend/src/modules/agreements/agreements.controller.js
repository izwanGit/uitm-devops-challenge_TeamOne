...
exports.generateAgreement = async (req, res) => {
    try {
        ...
        if (req.user && (req.user.id !== lease.tenantId && req.user.id !== lease.landlordId && req.user.role !== 'ADMIN')) {
            return res.status(403).json({ error: 'Unauthorized access to this lease' });
        }
...
exports.signAgreement = async (req, res) => {
    try {
        ...
        if (req.user && req.user.id !== agreement.lease.tenantId) {
            return res.status(403).json({ error: 'Only the tenant can sign this agreement' });
        }
