...
async function analyzeRisk(userId, context) {
    ...
    try {
        ...
        if (recentEvents.length > 0) {
            const knownFingerprints = new Set(recentEvents.map(e => e.deviceFingerprint).filter(Boolean));
            if (knownFingerprints.size > 0 && !knownFingerprints.has(context.deviceFingerprint)) {
                score += 10;
        ...
        if (recentEvents.length > 0 && context.lat && context.long) {
            ...
            if (lastEvent.geoLat && lastEvent.geoLong) {
                ...
                const timeDiff = (Date.now() - new Date(lastEvent.createdAt).getTime()) / (1000 * 60 * 60); // hours
