const { analyzeRisk, getGeoContext, generateDeviceFingerprint } = require('../utils/heuristic.util');
...
const detectAnomaly = (eventType) => {
  return async (req, res, next) => {
    try {
      ...
      const userId = req.user ? req.user.id : (req.body.email ? 'PRE_AUTH' : 'ANONYMOUS');
      ...
      // Analyze Risk
      const { score, reasons } = await analyzeRisk(targetUserId, currentContext);
      ...
      if (score > 60) {
          return res.status(403).json({
              ...
              details: 'Account temporarily locked. Please verify identity.',
              riskScore: score
          });
      }
