...
class AIService {
  ...
  normalizePropertyType(propertyType) {
    ...
    if (normalized.includes('apartment') || normalized.includes('apt'))
      return 'Apartment';
    if (normalized.includes('service') && normalized.includes('residence'))
    ...
    console.log(
      `⚠️ Unknown property type "${propertyType}", defaulting to Condominium`
    );
    return 'Condominium';
  }
  ...
  async classifyPropertyApproval(propertyData) {
    try {
      ...
      const normalizedPropertyType = this.normalizePropertyType(
        propertyData.propertyType
      );

      console.log(
        '🤖 AI Service - Sending request to:',
        `${this.baseUrl}/api/v1/classify/approval`
      );
      console.log(
        '🤖 AI Service - Property type mapping:',
        propertyData.propertyType,
        '->',
      ...
      return {
        ...
        reason:
          result.approval_reasons?.join(', ') || 'AI classification completed',
