...
const handleAlerts = async (user, riskAnalysis, req, status = 'SUCCESS') => {
    const { riskScore, severity, reasons, deviceHash, geo, metaData } = riskAnalysis;
    ...
    if (severity === 'CRITICAL' || status === 'BLOCKED') {
        console.warn(`[SECURITY] CRITICAL ALERT for User ${user.email}: ${reasons}`);
        ...
        try {
            await emailService.sendEmail(
                ...
                'security_alert',
                {
                    ...
                    location: location,
                    time: time,
                    action: 'Your account has been temporarily locked. Please contact support.',
                }
            );
        } catch (err) {
