...
const runVerification = async () => {
  ...
  try {
    ...
    console.log(
      `RISK SCORE: ${resultA.riskScore} | SEVERITY: ${resultA.severity}`
    );

    // Log Success
    await handleAlerts(user, resultA, reqA, 'SUCCESS');

    if (resultA.severity === 'SAFE')
      console.log('✅ PASS: Initial login is SAFE');
    ...
    console.log(
      `RISK SCORE: ${resultB.riskScore} | SEVERITY: ${resultB.severity}`
    );

    // Log Success
    await handleAlerts(user, resultB, reqB, 'SUCCESS');

    if (resultB.riskScore >= 20 && resultB.severity === 'SUSPICIOUS')
      console.log('✅ PASS: New Device flagged as SUSPICIOUS');
    ...
    console.log(
      `RISK SCORE: ${resultC.riskScore} | SEVERITY: ${resultC.severity}`
    );
    console.log(`REASONS: ${resultC.reasons}`); // Should show speed

    if (
      resultC.severity === 'CRITICAL' &&
      resultC.reasons.includes('Impossible Travel')
    ) {
