        ...
        while (true) {
            ...
            for (const p of properties) {
                ...
                if (Array.isArray(p.images) && p.images.length === 1) {
                    ...
                    if (typeof firstImg === 'string' && firstImg.includes(',') && (firstImg.match(/http/g) || []).length > 1) {
                        ...
                        newImages = firstImg.split(',').map(s => s.trim()).filter(s => s.length > 0)
                        needsFix = true
                    }
                    ...
                    for (const img of p.images) {
                        if (typeof img === 'string' && img.includes(',') && (img.match(/http/g) || []).length > 1) {
                            const splitImgs = img.split(',').map(s => s.trim()).filter(s => s.length > 0)
                ...
                if (needsFix) {
                    // Push update promise
                    updates.push(prisma.property.update({
                        where: { id: p.id },
